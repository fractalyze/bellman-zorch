...
typedef enum {
  ...
  // Wide-int storage tags. Opaque byte buffers (BigInt<N>-shaped) for
  // prime-ir field / EC element storage; arithmetic lives in the MLIR
  // pipeline, not here.
  PJRT_Buffer_Type_U128 = 138,
  PJRT_Buffer_Type_U256 = 139,
  PJRT_Buffer_Type_S128 = 128,
  PJRT_Buffer_Type_S256 = 129,

  // ZK field/EC PrimitiveTypes. Field semantic (modulus, Montgomery flag)
  // lives in the MLIR type carried alongside the buffer; the C API just
  // round-trips the storage bytes.
  PJRT_Buffer_Type_BABYBEAR = 150,
  PJRT_Buffer_Type_BABYBEAR_MONT = 151,
  PJRT_Buffer_Type_KOALABEAR = 152,
  PJRT_Buffer_Type_KOALABEAR_MONT = 153,
  PJRT_Buffer_Type_MERSENNE31 = 154,
  PJRT_Buffer_Type_GOLDILOCKS = 155,
  PJRT_Buffer_Type_GOLDILOCKS_MONT = 156,
  PJRT_Buffer_Type_BN254_SF = 157,
  PJRT_Buffer_Type_BABYBEARX4 = 170,
  PJRT_Buffer_Type_BN254_G1_AFFINE = 190,
  PJRT_Buffer_Type_BN254_G1_JACOBIAN = 191,
  PJRT_Buffer_Type_BN254_G1_XYZZ = 192,
  // Mont / additional extension-field variants.
  PJRT_Buffer_Type_BN254_SF_MONT = 158,
  // ZK binary tower fields GF(2^(2^level)), level 0..7.
  PJRT_Buffer_Type_BINARY_FIELD_T0 = 159,
  PJRT_Buffer_Type_BINARY_FIELD_T1 = 160,
  PJRT_Buffer_Type_BINARY_FIELD_T2 = 161,
  PJRT_Buffer_Type_BINARY_FIELD_T3 = 162,
  PJRT_Buffer_Type_BINARY_FIELD_T4 = 163,
  PJRT_Buffer_Type_BINARY_FIELD_T5 = 164,
  PJRT_Buffer_Type_BINARY_FIELD_T6 = 165,
  PJRT_Buffer_Type_BINARY_FIELD_T7 = 166,
  PJRT_Buffer_Type_KOALABEARX4 = 172,
  PJRT_Buffer_Type_KOALABEARX4_MONT = 173,
  PJRT_Buffer_Type_BABYBEARX4_MONT = 171,
  PJRT_Buffer_Type_MERSENNE31X2 = 174,
  // BN254 EC point Mont variants. Same storage widths as canonical.
  PJRT_Buffer_Type_BN254_G1_AFFINE_MONT = 196,
  PJRT_Buffer_Type_BN254_G1_JACOBIAN_MONT = 197,
  PJRT_Buffer_Type_BN254_G1_XYZZ_MONT = 198,
  PJRT_Buffer_Type_BN254_G2_AFFINE_MONT = 199,
  PJRT_Buffer_Type_BN254_G2_JACOBIAN_MONT = 200,
  PJRT_Buffer_Type_BN254_G2_XYZZ_MONT = 201,
  // Canonical (non-Mont) G2. Storage widths 1024/1536/2048 — appended at
  // the end rather than placed near G1 canonical to keep this enum
  // append-only across PJRT releases (the slot index is the ABI; the
  // proto enum value is independent and carried via xla::PrimitiveType).
  PJRT_Buffer_Type_BN254_G2_AFFINE = 193,
  PJRT_Buffer_Type_BN254_G2_JACOBIAN = 194,
  PJRT_Buffer_Type_BN254_G2_XYZZ = 195,
  // Goldilocks degree-3 extension field. 192-bit storage.
  PJRT_Buffer_Type_GOLDILOCKSX3 = 175,
  PJRT_Buffer_Type_GOLDILOCKSX3_MONT = 176,
  // Pasta curves (Pallas/Vesta). Non-pairing, G1 only. The enum value is the
  // xla::PrimitiveType value (the ABI slot is the value, kept append-only).
  PJRT_Buffer_Type_PALLAS_SF = 177,
  PJRT_Buffer_Type_PALLAS_SF_MONT = 178,
  PJRT_Buffer_Type_VESTA_SF = 179,
  PJRT_Buffer_Type_VESTA_SF_MONT = 180,
  PJRT_Buffer_Type_PALLAS_G1_AFFINE = 202,
  PJRT_Buffer_Type_PALLAS_G1_JACOBIAN = 203,
  PJRT_Buffer_Type_PALLAS_G1_XYZZ = 204,
  PJRT_Buffer_Type_VESTA_G1_AFFINE = 205,
  PJRT_Buffer_Type_VESTA_G1_JACOBIAN = 206,
  PJRT_Buffer_Type_VESTA_G1_XYZZ = 207,
  PJRT_Buffer_Type_PALLAS_G1_AFFINE_MONT = 208,
  PJRT_Buffer_Type_PALLAS_G1_JACOBIAN_MONT = 209,
  PJRT_Buffer_Type_PALLAS_G1_XYZZ_MONT = 210,
  PJRT_Buffer_Type_VESTA_G1_AFFINE_MONT = 211,
  PJRT_Buffer_Type_VESTA_G1_JACOBIAN_MONT = 212,
  PJRT_Buffer_Type_VESTA_G1_XYZZ_MONT = 213,
} PJRT_Buffer_Type;
